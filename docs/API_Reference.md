...
| eReport | `{{base}}` | Bearer `{{integration_token}}`, then `X-EReport-View-Token: {{integration_report_view_token}}` for report reads |
| FACE LIVENESS | `{{baseUrl}}` | Header `x-api-key: {{apiKey}}` (lowercase) |
| DBM COMPASS | `{{baseUrl}}` | Header `X-API-Key: {{apiKey}}` (uppercase) |

> **Header-casing gotcha:** Compass uses `X-API-Key`; Face Liveness uses `x-api-key`. They are otherwise both keyed by `{{apiKey}}`.
...
- **Description:** Let citizens file and track complaints/reports: submit a complaint, verify by OTP, then list and view report status by case number.
- **Base:** `{{base}}` · common prefix `/api/integration/`
- **Token chaining:** `access_code` → `POST /token` yields `{{integration_token}}` (Bearer, used for datasets + submit + OTP). After OTP confirm you additionally receive a `report_view_token` (`{{integration_report_view_token}}`) — required in the `X-EReport-View-Token` header for reading reports.

### Report Type List
`GET {{base}}/api/integration/datasets/report_types`

- **Auth:** Bearer `{{integration_token}}` · no parameters.
- **Response:** JSON:API `data[]` of `report_types` (`code`, `name`, `sequence`, `is_visible`, `is_active`, `created_at`, plus nullable `img_url`/`icon_url`/`description`/`agency_name`/`hotline`), with `meta.pagination`. Example returns 9 types: `crime`, `red_tape`, `scam`, `child_abuse`, `women_abuse`, `overpricing`, `fire`, `accident`, `gas_station_concerns`.
...
- **Auth:** Bearer `{{integration_token}}` · no parameters.
- **Response:** `data[]` of `regions`; `id` = 9-digit PSA region code, `attributes.name`.
...
- **Query:** `region_code` (string, **required**) — PSA region code.
- **Response:** `data[]` of `provinces`; `id` = province code; `attributes`: `region_code`, `name`, `district`.
...
- **Auth:** Bearer `{{integration_token}}`
- **Query:** `province_code` (string, **required**) — PSA province code.
- **Response:** `data[]` of `municipalities`; `id` = municipality code; `attributes`: `region_code`, `province_code`, `name`, `zip_code`.
...
- **Auth:** Bearer `{{integration_token}}`
- **Query:** `municipality_code` (string, **required**) — PSA municipality code.
- **Response:** `data[]` of `barangays`; `id` = barangay code; `attributes`: `region_code`, `province_code`, `municipality_code`, `name`, `zip_code`.
...
- **Auth:** none (access code in body). Test script stores the result into `integration_token`.

| Field | Type | Required | Description |
|---|---|---|---|
| `access_code` | string | Yes | Pre-issued access code (`{{access_code}}`). |
...
| `report_type` | string | Yes | e.g. `crime` (see Report Type List). |
| `subject` | string | Yes | |
| `message` | string | Yes | |
...
**Response:** `code` (200), `message`, `case_number` (e.g. `PFM-071826-0014`).
...
```
> The source cURL contains a stray malformed empty header `--header ': '` — omit it.
...
Sends a 6-digit OTP to an email (expires in 5 minutes).

- **Auth:** Bearer `{{integration_token}}`

| Field | Type | Required | Description |
|---|---|---|---|
| `email` | string | Yes | Address to receive the OTP. |

**Response:** `code` (200), `already_verified` (bool), `message`.
...
Confirms the OTP and returns a `report_view_token` (test script stores it into `integration_report_view_token`).
...
**Response:** `code` (200), `report_view_token` (UUID), `expires_at`.
...
- **Auth:** Header `X-EReport-View-Token: {{integration_report_view_token}}` (**not** Bearer).
...
**Response:** JSON:API `data[]` of `reports` — `case_number`, `complainant` (name/phone/gender/email), `report_type`, `subject`, `message`, `evidences[]`, `address` (region/province/municipality/barangay code+name, lat/long, country), `status`, `formatted_status`, `history[]`, `created_at`; plus `meta.pagination`.
...
- **Auth:** Header `X-EReport-View-Token: {{integration_report_view_token}}` (required).
- **Path param:** `case_number` (string, required) — e.g. `PFM-071826-0014`.
- **Response:** single `data` object with the same shape as a Reports List item; here `history[]` is populated (`status`, `formatted_status`, `remarks`, `created_at`).
...
> **Source caveat:** `ereport.md` contains a duplicated/erroneous "Report List" block that actually repeats the Region List docs verbatim. The real report-listing endpoint is the "Reports List" entry above (`/api/integration/reports`).
...
- **Description:** Centralized Open Monitoring Platform for Appropriations and Spending Statistics — programmatic access to public DBM budget-execution data (SAAODB, NCA, SARO, LGSF records and dashboard summaries).
- **Base:** `{{baseUrl}}` · common prefix `/api/v1/records/`
- **Auth (all endpoints):** Header `X-API-Key: {{apiKey}}` (uppercase).

### Get SAAODB Records
`GET {{baseUrl}}/api/v1/records/saaodb`

Paginated SAAODB (Statement of Appropriations, Allotments, Obligations, Disbursements, and Balances) records.

| Query | Type | Required | Description |
|---|---|---|---|
| `reportYear` | integer | Yes | Fiscal report year (e.g. 2026). |
| `period` | string | Yes | One of `Q1`, `Q2`, `Q3`, `Q4`, `FY`. |
...
`GET {{baseUrl}}/api/v1/records/saaodb/dashboard`

Aggregated budget-execution summary for a year/scope.
...
**Response:** `reportYear`, `sheetScope`, `cascade` (`appropriations`, `adjustments`, `totalAvailable`, `allotments`, `obligations`, `unobligated`, `disbursements`, `unreleased`), `rates` (`obligationRate`, `disbRateOblig`, `disbRateAppro`), `classBreakdown[]` (`class`, `amount`), `appropriationSplit` (`currentYear`, `continuing`, `hasSplit`), `topEntities[]` (may be empty for summary scope).

```bash
curl --request GET \
  --url '{{baseUrl}}/api/v1/records/saaodb/dashboard?reportYear=2026&sheetScope=summary' \
  --header 'X-API-Key: {{apiKey}}'
```
...
```json
{
  ...
  "rates": { "obligationRate": 0.28938065, "disbRateOblig": 0.79530579 },
  "classBreakdown": [
    { "class": "PS", "amount": 396271169433.72 },
    { "class": "MOOE", "amount": 755239194473.85 }
  ],
  "appropriationSplit": { "currentYear": 6793162000000, "continuing": 653794425179.78, "hasSplit": true },
  "topEntities": []
}
```

### Get SAAODB Hierarchical Entities
`GET {{baseUrl}}/api/v1/records/saaodb/entities`

Navigate department → agency → fund-source relationships.

| Query | Required | Description |
|---|---|---|
| `reportYear` | Yes | Fiscal report year. |
| `sheetScope` | Yes | `agency`, `sucs` (note: `summary` not listed here). |
| `expandParent` | No | Department name to expand into child agencies. |
| `expandEntity` | No | Agency name to expand into fund sources. |
| `expandEntityParent` | No | Parent department name to disambiguate agencies sharing a name. |

```bash
curl --request GET \
  --url '{{baseUrl}}/api/v1/records/saaodb/entities?reportYear=2026&sheetScope=agency&expandParent=Department of Finance' \
  --header 'X-API-Key: {{apiKey}}'
```

### Get NCA Records
`GET {{baseUrl}}/api/v1/records/nca`
...
| `expenseClass` | string | No | Expense class UACS code (e.g. `5020000000`). |
...
```bash
curl --request GET \
  --url '{{baseUrl}}/api/v1/records/nca?budgetYear=2026&deptCode=010000000000&agencyCode=010010000000&operatingUnitCode=010010000001&expenseClass=5020000000&page=1&limit=100' \
  --header 'X-API-Key: {{apiKey}}'
```

### Get SARO Records
`GET {{baseUrl}}/api/v1/records/saro`
...
**Response:** `data[]`, `total`, `page`, `limit`. Each SARO record: `saroNo`, `deptCode`, `agencyCode`, `expenseClass`, `amount`, `dateIssued` (ISO 8601).

```bash
curl --request GET \
  --url '{{baseUrl}}/api/v1/records/saro?saroNo=SARO-BMB-A-26-0000001&deptCode=010000000000&agencyCode=010010000000&expenseClass=5020000000&page=1&limit=100' \
  --header 'X-API-Key: {{apiKey}}'
```

### Get LGSF Records
`GET {{baseUrl}}/api/v1/records/lgsf`

Paginated Local Government Support Fund records. **All params optional** (omitting all returns everything, subject to default pagination).

| Query | Description |
|---|---|
| `fiscalYear` | Program budget year. |
| `programCode` | LGSF program: `FALGU`, `GEF`, `GGG`, `SBDP`, `SAFPB`. |
| `regionCode` | Region UACS/geo code (e.g. `PH030000000`). |
| `province` | Province name. |
| `cityMunicipality` | City/municipality name. |
| `page` | Page number. |
| `limit` | Page size. |

```bash
curl --request GET \
  --url '{{baseUrl}}/api/v1/records/lgsf?fiscalYear=2026&programCode=FALGU&regionCode=PH030000000&province=Bulacan&cityMunicipality=Malolos&page=1&limit=100' \
  --header 'X-API-Key: {{apiKey}}'
```

### Get LGSF Dashboard Summary
`GET {{baseUrl}}/api/v1/records/lgsf/dashboard`

KPIs, yearly trend, and a paginated project list for an LGSF program.

| Query | Required | Description |
|---|---|---|
| `programCode` | Yes | `FALGU`, `GEF`, `GGG`, `SBDP`, `SAFPB`. |
| `reportYear` | No | Fiscal year. |
...
**Response:** `programCode`, `reportYear`, `kpis` (`totalReleased`, `projectCount`, `lguCount`, `barangayCount`, `regionCount`, `provinceCount`, `fiscalYearCount`), `trend[]`, `projects` (`rows[]`, `total`, `page`, `pageSize`).

```bash
curl --request GET \
  --url '{{baseUrl}}/api/v1/records/lgsf/dashboard?programCode=FALGU&reportYear=2026&region=Region III&province=Bulacan&municipality=Malolos&page=1&limit=25' \
  --header 'X-API-Key: {{apiKey}}'
```

> **Param-naming caveats (Compass):** LGSF **Records** uses `fiscalYear` + `cityMunicipality`, whereas the LGSF **Dashboard** uses `reportYear` + `municipality`. Mind the difference when reusing query strings across the two.
