...
import { Badge, Button, ButtonLink, Card, CardBody, CardHeader, EmptyState, PageHeader, StatusBadge, inputClass } from '@/components/ui'
...
type ReviewService = Record<string, unknown> & { id: string; display_name: string; version: number; status: string; source_prompt: string | null; fee_amount: number; waivers: unknown; eligibility: unknown; form_fields: unknown; approval_office: string | null; generation_confidence: number | null; lgu: { name: string }; template: { name: string }; flags: ReviewFlag[] }
...
export default async function ReviewPage() {
  ...
  return <div className="space-y-6"><PageHeader eyebrow="DICT reviewer" title="Validation review queue" description="Approve exceptions with an audit note, reject unsafe configurations, and publish only after every blocking finding is resolved." action={<ButtonLink href="/citizen/services" variant="secondary">Open citizen catalog</ButtonLink>} />
    {!services.length ? <Card><EmptyState title="No flagged services" description="Template-conforming services publish without entering this queue." /></Card> : services.map((service) => {
      ...
      return <Card key={service.id}><CardHeader title={`${service.display_name || service.template.name} · ${service.lgu.name}`} description={`Version ${service.version} · ${service.source_prompt || 'Configured manually'}`} action={<StatusBadge status={service.status} />} />
