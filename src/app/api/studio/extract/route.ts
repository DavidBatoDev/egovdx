...
export async function POST(request: Request) {
  try {
    ...
    if (!prompt) return Response.json({ error: 'Describe the eService before generating' }, { status: 400 })
    ...
    const generation = await generateServiceFromExtraction(extraction.data, session.lguId, prompt)
