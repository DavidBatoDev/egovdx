...
export function TrackClient({ request, events }: { request: RequestWithService; events: RequestEvent[] }) {
  ...
  return <main className="mx-auto max-w-2xl space-y-6 px-4 py-8"><PageHeader eyebrow={request.service.lgu.name} title={request.service.template.name} description={`Pending ${request.service.approval_office ?? 'LGU'} approval`} action={<StatusBadge status={request.status} />} />
