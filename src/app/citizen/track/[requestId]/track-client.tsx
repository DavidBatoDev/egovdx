...
import { Badge, ButtonAnchor, ButtonLink, Card, CardBody, CardHeader, PageHeader, SourceBadge, StatusBadge } from '@/components/ui'
...
export function TrackClient({ request, events }: { request: RequestWithService; events: RequestEvent[] }) {
  ...
  return <div className="mx-auto max-w-2xl space-y-6 px-4 py-8"><PageHeader eyebrow={request.service.lgu.name} title={request.service.display_name || request.service.template.name} description={description} action={<StatusBadge status={request.status} />} />
    ...
    {request.status === 'issued' ? <div className="flex flex-wrap gap-3"><ButtonAnchor href={`/api/issue/download?id=${request.id}`}>Download PDF</ButtonAnchor><ButtonLink variant="secondary" href={`/verify/${request.id}`}>Verify document</ButtonLink></div> : null}
  </div>
}
