...
export function ServicesClient({ lgus }: { lgus: CitizenLgu[] }) {
  ...
  return (
    <div className="min-h-[510px]">
      <div className="p-1">
        {view === 'home' ? <div className="flex min-h-[390px] flex-col justify-between"><div><p className="text-sm text-muted">Good day</p><h2 className="mt-1 font-display text-3xl text-foreground">What can we help you do?</h2></div><button type="button" onClick={() => setView('lgus')} className="flex w-full items-center justify-between rounded-xl bg-white p-5 text-left shadow-sm ring-1 ring-border transition hover:ring-brand"><span><span className="block text-lg font-bold text-foreground">LGUs</span><span className="mt-1 block text-sm text-muted">Find services from your local government</span></span><span className="flex h-11 w-11 items-center justify-center rounded-full bg-brand text-xl font-bold text-white">→</span></button><p className="text-center text-xs text-muted">eGovPH services are provided securely by participating agencies.</p></div> : null}
        ...
        {view === 'services' && selectedLgu ? <div className="space-y-5"><button type="button" onClick={() => setView('lgus')} className="text-sm font-bold text-brand">← LGUs</button><div><p className="text-sm font-bold uppercase tracking-wide text-brand">{selectedLgu.type}</p><h2 className="mt-1 font-display text-3xl text-foreground">{selectedLgu.name}</h2><p className="mt-2 text-sm text-muted">Available eServices</p></div>{selectedLgu.services.length ? <div className="space-y-3">{selectedLgu.services.map((service) => <Card key={service.id} className="bg-white"><CardBody className="space-y-3"><div className="flex items-start justify-between gap-3"><div><h3 className="font-bold text-foreground">{service.template.name}</h3>{service.template.description ? <p className="mt-1 text-sm text-muted">{service.template.description}</p> : null}</div><Badge tone={service.fee_amount > 0 ? 'accent' : 'success'}>{service.fee_amount > 0 ? peso(service.fee_amount) : 'Free'}</Badge></div><div className="flex items-center justify-between gap-3"><p className="text-xs text-muted">{service.required_docs.length} supporting document{service.required_docs.length === 1 ? '' : 's'} required</p><ButtonLink href={`/citizen/apply/${service.id}`}>Apply</ButtonLink></div></CardBody></Card>)}</div> : <Card className="rounded-xl bg-white"><CardBody className="py-10 text-center"><p className="font-semibold text-foreground">No eServices found</p><p className="mt-2 text-sm text-muted">This LGU has not published online services yet. Please check back later.</p></CardBody></Card>}</div> : null}
      </div>
    </div>
  )
}
