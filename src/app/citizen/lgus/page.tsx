...
import { listPublishedLguSites } from '@/lib/lgu-site/data'

export const dynamic = 'force-dynamic'

export default async function LgusPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  const query = (await searchParams).q?.trim().toLowerCase() ?? ''
  const [services, sites] = await Promise.all([listPublishedServices(), listPublishedLguSites()])
  const groups = new Map<string, { id: string; name: string; count: number }>()
  for (const site of sites) groups.set(site.lgu.id, { id: site.lgu.id, name: site.lgu.name, count: 0 })
