        ...
        <Card className="border-danger/30 bg-danger-soft">
          <CardBody className="space-y-2 text-sm text-danger">
            {error === 'database_unavailable' ? (
              ...
              <><p className="font-medium">Start sign-in from eGovPH.</p><p>The published integration begins when eGovPH hands this service a short-lived exchange code. No undocumented authorization URL is used.</p></>
            ) : error === 'missing_exchange_code' ? (
              <p>The eGovPH handoff did not include an exchange code. Generate a fresh single-use code and try again.</p>
            ) : error === 'sso_unavailable' ? (
              <p>eGovPH sign-in is temporarily unavailable. No fallback session was created.</p>
            ) : (
