...
import { Badge, Button, Card, CardBody, CardHeader, Field, SourceBadge, inputClass } from '@/components/ui'
  ...
  return <div className="space-y-5">
    ...
    {result ? <>
      ...
      {!harness ? <Card><CardHeader title="4. Officer confirmation" description="One atomic save writes the service and validation findings." />
        <CardBody className="flex items-center gap-4"><Button disabled={busy} onClick={confirm}>Confirm and submit</Button>{saved ? <p className="text-sm"><strong>{saved.status === 'published' ? 'Published' : 'Sent to DICT review'}</strong> · service {saved.serviceId}</p> : null}</CardBody></Card> : null}
